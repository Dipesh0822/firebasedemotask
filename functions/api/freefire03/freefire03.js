    ...
    (async () => {
        try {
            ...
            if(dType.toUpperCase() === "ANDROID") {
                const androidAds = db.collection('freefire03');
                ...
                androidAdsData.forEach(doc => {
                    ...
                    } else if(doc.id === "ad_id")  {
                       dataAndroid[doc.id] = doc.data().ad_id
                    } else if(doc.id === "keyword")  {
                        dataAndroid[doc.id] = doc.data().keyword
                     }
                });
                resData = {
                    "status": true,
