    ...
    (async () => {
        try {
            ...
            if (dType.toUpperCase() === "ANDROID") {
                const androidAds = db.collection('test');
...
router.get('/', (req, res) => {
    (async () => {
        try {
            ...
            await db.collection("wallpaper01").doc('ad_id').set({
            ...
            await db.collection('wallpaper01').doc('keyword').set(keyword);
            await db.collection('wallpaper01').doc('more_app').set({ more_app: more_app });
            await db.collection('wallpaper01').doc('play_game').set({ play_game: play_game });
            resData = {
                "status": true
            }
            return res.status(200).send(resData);
